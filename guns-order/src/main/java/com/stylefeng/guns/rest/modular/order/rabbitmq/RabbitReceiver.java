...
public class RabbitReceiver {
    ...
    public  void onMessage(Message message, Channel channel,CorrelationData correlationData)throws Exception{
        try{
            ...
            messageManage.setId(Integer.valueOf(messageId));
            messageManage.setOrderId(Integer.valueOf(orderId));
            messageManage.setStauts(1);
            messageManageMapper.insert(messageManage);

            channel.basicAck(deliveryTag,false);

        }catch (Exception e){
            MessageManage messageManage=new MessageManage();
            messageManage.setId((Integer) message.getHeaders().get("messageId"));
            messageManage.setOrderId((Integer) message.getPayload());
